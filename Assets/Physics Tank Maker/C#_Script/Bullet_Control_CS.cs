...
namespace ChobiAssets.PTM
{
    ...
    {
        public float impactAngle; // The angle the bullet impacted the surface at.
        public Bullet_Control_CS bulletType; // The type of bullet which impacted.
		...
		*/
        ...
        { // The collision has been detected by the physics engine.
            ...
            if (isLiving)
            {
                ...
                {
                    case 0: // AP
                        // AP_Hit_Process(collision.collider.gameObject, collision.relativeVelocity.magnitude, collision.GetContact(0).normal);
                        TargetHit(collision.collider.gameObject, collision.relativeVelocity.magnitude, collision);
                        break;
        ...
        private void TargetHit(GameObject hitObject, float hitVelocity, Collision collision)
        {
            ArmorRegion armorHit = hitObject.GetComponent<ArmorRegion>();
            Vector3 impactNormal = collision.GetContact(0).normal;
            float impactAngle = Mathf.Abs(180 - Vector3.Angle(collision.relativeVelocity, -impactNormal));

            if (armorHit)
            {
                armorHit.HandleImpact(new ImpactData(impactAngle, this, collision));
            }
        }
